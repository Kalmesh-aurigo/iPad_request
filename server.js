...
const ZENDESK_EMAIL     = process.env.ZENDESK_EMAIL     || 'admin@company.com';
...
app.post('/create-ticket', async (req, res) => {
  try {
    ...
    if (!data.name || !data.email || !data.subject) {
      return res.status(400).json({ error: 'Missing required fields: name, email, subject' });
    }
    ...
    console.log(`Ticket created: #${ticketId} for ${data.name} (${data.email})`);
